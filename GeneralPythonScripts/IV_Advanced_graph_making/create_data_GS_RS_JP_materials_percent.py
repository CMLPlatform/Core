...
def getfile():
    ...
    f=open('../../data/IV_Advanced_graph_making/JP_GAS_RS_materials_percent.csv', 'r')
...
def create_output(input):
    ...
    data_list = []
    #parse over the replace(lines
    for line in input:
        years = replace(line[0])
        timber_hist_jp = replace(line[1])
        timber_sc1_jp = replace(line[2])
        timber_sc2_jp = replace(line[3])
        #timber_sc3 = replace(line[4])
        iron_hist_jp = replace(line[6])
        iron_sc1_jp = replace(line[7])
        iron_sc2_jp = replace(line[8])
        #iron_sc3 = replace(line[9])
        other_hist_jp = replace(line[11])
        other_sc1_jp = replace(line[12])
        other_sc2_jp = replace(line[13])
        #other_sc3 = replace(line[14])
        minerals_hist_jp = replace(line[16])
        minerals_sc1_jp = replace(line[17])
        minerals_sc2_jp = replace(line[18])
        RS_timber_hist_jp = replace(line[21])
        RS_timber_sc1_jp = replace(line[22])
        RS_timber_sc2_jp = replace(line[23])
        # timber_sc3 = replace(line[4])
        RS_iron_hist_jp = replace(line[26])
        RS_iron_sc1_jp = replace(line[27])
        RS_iron_sc2_jp = replace(line[28])
        # iron_sc3 = replace(line[9])
        RS_other_hist_jp = replace(line[31])
        RS_other_sc1_jp = replace(line[32])
        RS_other_sc2_jp = replace(line[33])
        # other_sc3 = replace(line[14])
        RS_minerals_hist_jp = replace(line[36])
        RS_minerals_sc1_jp = replace(line[37])
        RS_minerals_sc2_jp = replace(line[38])
        #minerals_sc3 = replace(line[19])
