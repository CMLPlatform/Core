...
def getfile():
    ...
    f=open('/home/chai/Dropbox/CML/1software_dev/Django/CMLMasterProject/data/IV_Advanced_graph_making/US_GAS_RS_materials_percent.csv', 'r')
...
def create_output(input):
    ...
    for line in input:
        years = line[0]
        timber_hist = replace(line[1])
        timber_sc1 = replace(line[2])
        timber_sc2 = replace(line[3])
        #timber_sc3 = line[4]
        iron_hist = replace(line[6])
        iron_sc1 = replace(line[7])
        iron_sc2 = replace(line[8])
        #iron_sc3 = line[9]
        other_hist = replace(line[11])
        other_sc1 = replace(line[12])
        other_sc2 = replace(line[13])
        #other_sc3 = line[14]
        minerals_hist = replace(line[16])
        minerals_sc1 = replace(line[17])
        minerals_sc2 =replace(line[18])
        RS_timber_hist = replace(line[21])
        RS_timber_sc1 = replace(line[22])
        RS_timber_sc2 = replace(line[23])
        # timber_sc3 = line[4]
        RS_iron_hist = replace(line[26])
        RS_iron_sc1 = replace(line[27])
        RS_iron_sc2 = replace(line[28])
        # iron_sc3 = line[9]
        RS_other_hist = replace(line[31])
        RS_other_sc1 = replace(line[32])
        RS_other_sc2 = replace(line[33])
        # other_sc3 = line[14]
        RS_minerals_hist = replace(line[36])
        RS_minerals_sc1 = replace(line[37])
        RS_minerals_sc2 = replace(line[38])
