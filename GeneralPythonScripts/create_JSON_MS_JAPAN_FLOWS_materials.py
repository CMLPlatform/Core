...
def getfile():
    ...
    f=open('../data/JAPAN_GAS_materials.csv', 'r')
...
def getfile2():
    #open the file
    #*** > give the path to the file.
    f=open('../data/JAPAN_RS_materials.csv', 'r')
...
def create_output(input):
    ...
    for line in input:
        ...
        if years <= '2005':
            ...
            json_string += '"'+str(id)+'":{"scenarios":"GAS_Historical_Series_jp","year":'+years+\
...
def create_output2(input):
    ...
    for line in input:
        ...
        if years <= '2005':
            ...
            json_string += '"'+str(id)+'":{"scenarios":"RS_Historical_Series_jp","year":'+years+\
                           ',"materials":[["timber",'+timber_hist+'],["iron",'+iron_hist+'],["other metals",'\
                           +other_hist+'],["minerals",'+minerals_hist+']]},'
        #after 2005 we have two scenarios we want to use
        else:
            ...
            #be careful with parsing here
            json_string += '"' + str(id) + '":{"scenarios":"RS_sc1_jp","year":' + years + \
                       ',"materials":[["timber",' + timber_sc1 + '],["iron",' + iron_sc1 + '],["other metals",' \
                       + other_sc1 + '],["minerals",' + minerals_sc1 + ']]},'
            id2 = id2 + 1
            json_string += '"' + str(id2) + '":{"scenarios":"RS_sc2_jp","year":' + years + \
                       ',"materials":[["timber",' + timber_sc2 + '],["iron",' + iron_sc2 + '],["other metals",' \
                       + other_sc2 + '],["minerals",' + minerals_sc2 + ']]},'
            id3 = id3 + 1
            json_string += '"' + str(id3) + '":{"scenarios":"RS_sc3_jp","year":' + years + \
                       ',"materials":[["timber",' + timber_sc3 + '],["iron",' + iron_sc3 + '],["other metals",' \
                       + other_sc3 + '],["minerals",' + minerals_sc3 + ']]},'
