...
def getfile():
    ...
    f=open('/home/chai/Dropbox/CML/1software_dev/Django/CMLMasterProject/data/JAPAN_stockflows_tonnes_MS_growth.csv', 'r')
    ...
    for line in input:
        if line[0] != None:
            ...
            if MS_sc1 == '' and MS_sc2 == '':
                temp_output = '{"date" : "'+years+'-01-01"'+', "MS_total_hist_jp" : '+MS_hist+'},'
                data_list.append(temp_output)
            #if no historical data present do not deal with those elements
            if MS_hist == '':
                temp_output = '{"date" : "'+years+'-01-01"'+',"MS_total_sc1_jp" :'+MS_sc1+',"MS_total_sc2_jp" : '+MS_sc2+'},'
                data_list.append(temp_output)
            # in 2005 all data elements are present
            if years == '2005':
                temp_output = '{"date" : "' + years + '-01-01"' + ', "MS_total_hist_jp" : ' + MS_hist + ',"MS_total_sc1_jp" :' + MS_sc1 + ',"MS_total_sc2_jp" : ' + MS_sc2 + '},'
                data_list.append(temp_output)
